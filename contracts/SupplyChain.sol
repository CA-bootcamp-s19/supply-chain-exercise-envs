...
contract SupplyChain {
    ...
    */
    ...
    modifier verifyCaller (address _address) {
        require (msg.sender == _address, "Only verified users can call this function");
        _;
    }
    modifier paidEnough(uint _price) {
        require(msg.value >= _price,"Enough money to make purchase");
        _;
    }
   ...
   */
    ...
    modifier forSale (uint _sku, State state) {
        require(uint(items[_sku].state) == uint(State.ForSale));
        _;
    }

    modifier sold (uint _sku, State state) {
        require(uint(items[_sku].state) == uint(State.Sold));
        _;
    }

    modifier shipped (uint _sku, State state) {
        require(uint(items[_sku].state) == uint(State.Shipped));
        _;
    }

    modifier received (uint _sku, State state) {
        require(uint(items[_sku].state) == uint(State.Received));
        _;
    }
